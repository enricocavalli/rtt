...
try {
	...
	if (empty($_SERVER['PATH_INFO'])) {
		throw new Exception('No PATH_INFO');
	}	
	...
	if ($modEnd === FALSE) {
		...
		throw new Exception('The URL must at least contain a module name followed by a slash.');
	}
	 ...
	 */
	if (strpos($url, '\\') !== FALSE) {
		throw new Exception('Requested URL contained a backslash.');
	} elseif (strpos($url, './') !== FALSE) {
		throw new Exception('Requested URL contained \'./\'.');
	}

	$moduleDir = RTT_Module::getModuleDir($module) . '/www/';
	...
	if (is_dir($path)) {
		 ...
		 * listings.
		 */
		throw new Exception('Directory listing not available.');
	}

	if (!file_exists($path)) {
		/* File not found. */
		throw new Exception('The URL wasn\'t found in the module.');
	}
